...
data_sets = [part_data_df, rankle_data_df]
...
for i in range(len(data_sets)):
    ...
    #rankle_edit_df = rankle_data_df.map(lambda x: x.lstrip('[ ').rstrip(' ]'))
    
    #Replace spaces between numbers with a comma
    part_edit2_df = part_edit_df.map(lambda x: x.replace("   ",",").replace("  ",",").replace(" ",","))
    #rankle_edit2_df = rankle_edit_df.map(lambda x: x.replace("   ",",").replace("  ",",").replace(" ",","))
...
plt.figure(2)
plt.plot(part_location_data[1], 'm')
plt.show()
